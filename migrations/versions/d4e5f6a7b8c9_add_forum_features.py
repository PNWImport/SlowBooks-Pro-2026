...
def upgrade() -> None:
    ...
    op.add_column(
        "payments",
        sa.Column("is_voided", sa.Boolean(), server_default="false", nullable=True),
    )

    # Vendor default expense account
    op.add_column(
        "vendors", sa.Column("default_expense_account_id", sa.Integer(), nullable=True)
    )
...
def downgrade() -> None:
    op.drop_constraint(
        "fk_vendor_default_expense_account", "vendors", type_="foreignkey"
    )
