...
def upgrade() -> None:
    ...
    op.add_column(
        "employees", sa.Column("residence_state", sa.String(2), nullable=True)
    )

    # -- Pay stubs: garnishments, reimbursements, per-stub work location --
    op.add_column(
        "pay_stubs",
        sa.Column("garnishments", sa.Numeric(12, 2), server_default="0", nullable=True),
    )
    op.add_column(
        "pay_stubs",
        sa.Column(
            "reimbursements", sa.Numeric(12, 2), server_default="0", nullable=True
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "deduction_type_id",
            sa.Integer(),
            sa.ForeignKey("deduction_types.id"),
            nullable=False,
        ),
    ...
    op.create_index(
        op.f("ix_employee_deductions_employee_id"),
        "employee_deductions",
        ["employee_id"],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "garnishment_type", enum_col("garnishmenttype"), server_default="CREDITOR"
        ),
    ...
    op.create_index(
        op.f("ix_garnishment_orders_employee_id"), "garnishment_orders", ["employee_id"]
    )
